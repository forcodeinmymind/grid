...
class Grid:
    ...
    def trunc_step(self, pos: tuple[int, int], step_size: tuple[int, int], offset: tuple[int, int]) -> tuple[int, int]:
        """truncate pos_px to step_size_px
        p.e. step_size = (cell_width, cell_height)
        """
        return ((pos[0] // step_size[0]) * step_size[0]) + offset[0], \
               ((pos[1] // step_size[1]) * step_size[1]) + offset[1]

    def is_coord_in_grid(self, coord: tuple[int | float, int | float]) -> bool:
